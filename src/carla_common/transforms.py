...
def carla_velocity_to_cyber_twist(carla_linear_velocity, carla_angular_velocity, carla_rotation=None):
    """
    ...
    cyber_twist = Twist()
    if carla_rotation:
        cyber_twist.linear.CopyFrom(carla_vector_to_cyber_vector_rotated(carla_linear_velocity, carla_rotation))
    else:
        cyber_twist.linear.CopyFrom(carla_location_to_cyber_vector3(carla_linear_velocity))
...
def carla_transform_to_cyber_transform(carla_transform):
    ...
    cyber_transform.translation.CopyFrom(carla_location_to_cyber_point3d(
        carla_transform.location))
    cyber_transform.rotation.CopyFrom(carla_rotation_to_cyber_quaternion(
...
def carla_transform_to_cyber_pose(carla_transform):
    ...
    cyber_pose.position.CopyFrom(carla_location_to_cyber_point(
        carla_transform.location))
    cyber_pose.orientation.CopyFrom(carla_rotation_to_cyber_quaternion(
        carla_transform.rotation))
...
def cyber_pose_to_transform_matrix(msg):
    ...
    """
    mat44 = numpy.eye(4)
    mat44[:3, :3] = quat2mat([msg.orientation.qw, msg.orientation.qx,
                              msg.orientation.qy, msg.orientation.qz])
...
def n2b(x_radius, y_radius, z_radius, b):
    ...
    n = numpy.matrix(numpy.array(b)) * numpy.matrix(x_matrix) * numpy.matrix(y_matrix) * numpy.matrix(z_matrix)
    return n
